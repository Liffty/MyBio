...
const MatrixIntro = ({ onIntroComplete }) => {
  ...
  useEffect(() => {
    ...
    const bannerCount = bannerImgSet.length + 1;
    let currentBannerIndex = 0;
    let bannerCycleCountdown = 0.2; //seconds per banner
    ...
    script.onload = () => {
      if (!window.createREGL || !canvasRef.current) return;
      ...
      const characterSet = ([]
        ...
        .concat(charRange(0x30a0, 0x30ff)) // katakana
      );
      ...
      let fadeCountdown = 0;
      let bannerCountdown = 6.0; // Reduced from 8.0 for faster intro
      let hasGlitched = false;
      let transitionToBlank = false;
      
      function renderWorld(delta) {
        // If we're transitioning to blank, clear everything and stop rendering
        if (transitionToBlank) {
          bufferContext.fillStyle = '#000';
          bufferContext.fillRect(0, 0, bufferW, bufferH);
          return;
        }
        ...
        // Redraw matrix trails
        bufferContext.textAlign = 'center';
        bufferContext.font = '12px "Inconsolata", monospace';
        trails.forEach((trail, index) => {
        ...
        if (bannerCountdown < 1.5 && !hasGlitched) {
          bannerCycleCountdown -= delta;
          bufferContext.fillStyle = `hsla(${180 + Math.random() * 220}, 100%, 30%, 1)`;
          bufferContext.fillRect(0, 0, bufferW, bufferH);

          if (currentBannerIndex < bannerImgSet.length) {
            // Draw image banner
            const img = bannerImgSet[currentBannerIndex];
            if (img.complete) {
              const imgW = 64; // or whatever width you want
              const imgH = 80;
              bufferContext.save();
              bufferContext.translate(bufferW / 2, bufferH / 2);
              //bufferContext.scale(2, 2); // Adjust scale as needed
              bufferContext.drawImage(img, -imgW / 2, -imgH / 2, imgW, imgH);
              bufferContext.restore();
            }
          } else {
            // Draw final "yyy" banner
            bufferContext.fillStyle = `hsla(${100 + Math.random() * 220}, 100%, 10%, 1)`;
            bufferContext.font = '50px Unna';
            bufferContext.save();
            bufferContext.scale(2, 1);
            bufferContext.fillText('LIFFTY', 120, 120);
            bufferContext.restore();
          }

          // Advance to next banner after countdown
          if (bannerCycleCountdown < 0) {
            currentBannerIndex++;
            bannerCycleCountdown = 0.5; // seconds per banner
          }

          // After last banner ("yyy"), finish
          if (currentBannerIndex >= bannerCount && bannerCountdown < 0) {
            hasGlitched = true;
            setTimeout(() => {
              transitionToBlank = true;
              setIsVisible(false);
              if (onIntroComplete) {
                onIntroComplete();
              }
      ...
      const termFgColor = hex2vector('#ffeeee');
      ...
      const quadCommand = regl({
        ...
        frag: `
          ...
          #define textureW 512.0
          #define textureH 256.0
          #define consoleW 496.0
          #define consoleH 256.0
          #define consolePadUVW 0.016129032
          #define consolePadUVH 0.03125
          #define charUVW 0.012096774
          #define charUVH 0.0390625
          
          void main() {
            vec2 consoleWH = vec2(496.0, 256.0);
            ...
            if (fromEdge.x > 0.0 && fromEdge.y > 0.0) {
              ...
              // Simulate 2x virtual pixel size
              ...
              vec2 uvAdjustment = inTexelOffset * vec2(0.0, 0.5 / consoleH);
      ...
      function rafBody() {
        ...
        animationId = requestAnimationFrame(rafBody);
      }
